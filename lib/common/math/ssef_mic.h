...
// Intel: namespace embree
{
  ...
  struct __align(16) ssef_m  // Intel: __aligned(16)
  {
    typedef sseb_m Mask;
    typedef ssei_m Int;
    ...
    __forceinline ssef_m( math::ZeroTy   ) { f[0] = f[1] = f[2] = f[3] = math::zero; }
    __forceinline ssef_m( math::OneTy    ) { f[0] = f[1] = f[2] = f[3] = math::one; }
    __forceinline ssef_m( math::NegInfTy ) { f[0] = f[1] = f[2] = f[3] = math::neg_inf; }
    __forceinline ssef_m( math::PosInfTy ) { f[0] = f[1] = f[2] = f[3] = math::pos_inf; }
    __forceinline ssef_m( math::StepTy   ) { f[0] = 0.0f; f[1] = 1.0f; f[2] = 2.0f; f[3] = 3.0f; }
    __forceinline ssef_m( math::NaNTy    ) { f[0] = f[1] = f[2] = f[3] = math::nan; }
    ...
    __forceinline const float& operator []( const size_t index ) const { MNRY_ASSERT(index < 4); return f[index]; }
    __forceinline       float& operator []( const size_t index )       { MNRY_ASSERT(index < 4); return f[index]; }
  };
  ...
  // MoonRay: added math:: namespace
  __forceinline const ssef_t sign      ( const ssef_t& a ) { return _mm512_mask_blend_ps(_mm512_cmp_ps_mask(a,ssef_t(ssef_m(math::zero)),_MM_CMPINT_LT), ssef_t(ssef_m(math::one)), -ssef_t(ssef_m(math::one))); }
