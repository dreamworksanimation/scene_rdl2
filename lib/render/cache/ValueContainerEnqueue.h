...
class ValueContainerEnqueue
//
// Enqueue data into memory buffer
//
{
public:
    explicit ValueContainerEnqueue(std::string* bytes) : mBuff(bytes) { reset(bytes->size()); }

    // These are shallow copies and this is intentional.
    // Purpose of ValueContainerEnqueue is enqueueing data into designated data memory and
    // we don't want to copy designated data memory when we do copy/move
    // ValueContainerEnqueue only defines enqueue operation. You need to consider designated
    // data memory management separately from ValueContainerEnqueue.
    ValueContainerEnqueue(const ValueContainerEnqueue&) noexcept = default;
    ValueContainerEnqueue(ValueContainerEnqueue&&) noexcept = default;

    virtual ~ValueContainerEnqueue() = default;

    inline void reset(const size_t offset = 0);
    ...
    {
        const size_t len = ValueContainerUtil::variableLengthLongMaxSize + sizeof(vec[0]) * vec.size();
        void* ptr = getEnqDataAddr(len);
        const size_t len2 = ValueContainerUtil::variableLengthEncoding(static_cast<unsigned long>(vec.size()), ptr);
        ptr = updatePtr(ptr, len2);
    ...
    inline void enqVLVec2ui(const math::Vec2<unsigned>& vec);
    ...
    inline size_t getCurrSeekOffset() const { return mId; } // for seek() operation
    inline void seek(size_t jumpSeekOffset); // seek jump to particular position
    inline void seekEnd(); // seek jump to end of data

    inline void shrinkToFit() { mBuff->resize(mId); } // resize to current mId but not change reserved capacity

    // Return total data size but you need extra caution if mStartId != 0 at construction. 
    // In this case, finalize() returns mData size and this is not equal of currentSize().
    ...
    static inline void memoryCopy(void* dest, const void* src, const size_t n) { std::memcpy(dest, src, n); }
...
private:
    ...
    void* getEnqDataAddrUpdate(const size_t len)
    {
        void* ptr = getEnqDataAddr(len);
        mId += len;
        return ptr;
    ...
    void expandBuff(size_t requestAddSize)
    {
        ...
        const size_t expandSizeOrg = requestAddSize - capacity() + mBuff->size();
    ...
    size_t mStartId {0};            // initial start position of mBuff
    size_t mId {0};                 // current data enqueue position of mBuff
    std::string *mBuff {nullptr};
...
inline void
ValueContainerEnqueue::reset(const size_t offset)
{
    mStartId = offset;
    mId = mStartId;
...
inline void
ValueContainerEnqueue::enqBool(const bool b)
{
...
inline void
ValueContainerEnqueue::enqChar(const char c)
{
...
inline void
ValueContainerEnqueue::enqUChar(const unsigned char uc)
{
...
inline void
ValueContainerEnqueue::enqUChar2(const unsigned char u0, const unsigned char u1)
{
...
inline void
ValueContainerEnqueue::enqUChar3(const unsigned char u0, const unsigned char u1, const unsigned char u2)
{
...
inline void
ValueContainerEnqueue::enqUChar4(const unsigned char u0, const unsigned char u1,
                                 const unsigned char u2, const unsigned char u3)
{
...
inline void
ValueContainerEnqueue::enqInt(const int i)
{
...
inline void
ValueContainerEnqueue::enqUInt(const unsigned int ui)
{
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthIntMaxSize);
...
inline void
ValueContainerEnqueue::enqLong(const long l)
{
...
inline void
ValueContainerEnqueue::enqULong(const unsigned long ul)
{
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthLongMaxSize);
...
inline void
ValueContainerEnqueue::enqFloat12(const float f0, const float f1, const float f2,
                                  const float f3, const float f4, const float f5,
                                  const float f6, const float f7, const float f8,
                                  const float f9, const float fa, const float fb)
{
...
inline void
ValueContainerEnqueue::enqString(const std::string& str)
{
    const unsigned long strLen = static_cast<unsigned long>(str.size());
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthLongMaxSize);
...
inline void    
ValueContainerEnqueue::enqByteData(const void* data, const size_t dataSize)
//
// only enqueue data itself, no size info
//
{
...
ValueContainerEnqueue::enqAlignPad(const unsigned short alignSize, const size_t addOffset)
{
    const size_t currSize = addOffset + currentSize() + sizeof(unsigned short);
...
inline void
ValueContainerEnqueue::enqBoolVector(const BoolVector& vec)
{
    const size_t len = ValueContainerUtil::variableLengthLongMaxSize + sizeof(char) * vec.size();
    void* ptr = getEnqDataAddr(len);
    const size_t len2 = ValueContainerUtil::variableLengthEncoding(static_cast<unsigned long>(vec.size()), ptr);
    ptr = updatePtr(ptr, len2);
...
inline void
ValueContainerEnqueue::enqStringVector(const StringVector& vec)
{
...
inline void
ValueContainerEnqueue::enqVLInt(const int i)
{
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthIntMaxSize);
    mId += ValueContainerUtil::variableLengthEncoding(i, ptr);
...
inline void
ValueContainerEnqueue::enqVLUInt(const unsigned int ui)
{
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthIntMaxSize);
    mId += ValueContainerUtil::variableLengthEncoding(ui, ptr);
...
inline void
ValueContainerEnqueue::enqVLLong(const long l)
{
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthLongMaxSize);
    mId += ValueContainerUtil::variableLengthEncoding(l, ptr);
...
inline void
ValueContainerEnqueue::enqVLULong(const unsigned long ul)
{
    void* ptr = getEnqDataAddr(ValueContainerUtil::variableLengthLongMaxSize);
    mId += ValueContainerUtil::variableLengthEncoding(ul, ptr);
...
inline void
ValueContainerEnqueue::enqVLVec2ui(const math::Vec2<unsigned>& vec)
{
    enqVLUInt(vec[0]);
    enqVLUInt(vec[1]);
}
...
inline void
ValueContainerEnqueue::enqVLIntVector(const IntVector& vec)
{
    void* ptr =
...
inline void
ValueContainerEnqueue::enqVLLongVector(const LongVector& vec)
{
    void* ptr =
        getEnqDataAddr(ValueContainerUtil::variableLengthLongMaxSize +
...
inline void
ValueContainerEnqueue::seek(size_t jumpSeekOffset)
{
    mId = jumpSeekOffset;
}

inline void
ValueContainerEnqueue::seekEnd()
{
    mId = mBuff->size();
}

inline size_t
ValueContainerEnqueue::finalize()
{
    const size_t size = currentSize();
    saveSizeT((void*)((uintptr_t)mBuff->data() + (uintptr_t)mStartId), size); // save total dataSize
    // debugDump("", "finalize()");
    shrinkToFit(); // resize to current mId but not change reserved capacity
...
inline void*
ValueContainerEnqueue::saveChar(void* ptr, const char c) const
{
...
inline void*
ValueContainerEnqueue::saveCharN(void* ptr, const char* c, const size_t n) const
{
...
inline void*
ValueContainerEnqueue::saveSizeT(void* ptr, const size_t t) const
{
    union {
        size_t t;
        char c[4];
