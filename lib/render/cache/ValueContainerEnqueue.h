...
{
public:
    explicit ValueContainerEnqueue(std::string *bytes) :
        mStartId(bytes->size()),
        mId(mStartId),
        mBuff(bytes)
    {
    ...
    {
        size_t len = ValueContainerUtil::variableLengthLongMaxSize + sizeof(vec[0]) * vec.size();
        void* ptr = getEnqDataAddr(len);
        size_t len2 = ValueContainerUtil::variableLengthEncoding(static_cast<unsigned long>(vec.size()), ptr);
...
{
    size_t len = ValueContainerUtil::variableLengthLongMaxSize + sizeof(char) * vec.size();
    void* ptr = getEnqDataAddr(len);
    size_t len2 = ValueContainerUtil::variableLengthEncoding(static_cast<unsigned long>(vec.size()), ptr);
    ptr = updatePtr(ptr, len2);
